...
import custom_util.logging as logging
import psutil
import torch
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    checkpoint_path = "{}/checkpoint-{:03d}.pth".format(args.fold_output_dir, epoch)
    to_save = {
...
def load_model(args, model_without_ddp, optimizer, loss_scaler):
    ...
    if args.init_ckpt:
        ...
        msg, _ = model_without_ddp.load_state_dict_to_backbone(checkpoint["model"])
    ...
    if args.resume:
        ...
        for k in ["head.weight", "head.bias", "pos_embed_spatial", "pos_embed_temporal",
                  "decoder_pos_embed_spatial", "decoder_pos_embed_temporal", "patch_embed.proj.weight", "patch_embed.proj.bias",
                  "decoder_pred.weight", "decoder_pred.bias"
                  ]:
            if (
                k in model_checkpoint
                and model_checkpoint[k].shape != state_dict[k].shape
            ):
                print(f"Removing key {k} from pretrained checkpoint")
                del model_checkpoint[k]
        if args.start_epoch == 0:
            del checkpoint["optimizer"]
            del checkpoint["epoch"]

        msg, _ = model_without_ddp.load_state_dict(model_checkpoint, strict=False)
        print('Missing keys: ', msg)
...
def untransform_image(image):
    return torch.clip((image.float() * IMG_STD + IMG_MEAN), 0, 1.0)
...
def get_visible_images(vars_: dict, model, save_dir: str, n_cut: int = 2):
    ...
    if isinstance(model, torch.nn.parallel.DistributedDataParallel):
        volumes = model.module.unpatchify(volumes)
        masks = masks.unsqueeze(-1).repeat(1, 1, vars_['reconstruct_imgs'].shape[-1])  # (N, H*W, p*p*3)
        masks = model.module.unpatchify(masks)  # 1 is removing, 0 is keeping
        pred_t_dim = model.module.pred_t_dim
    else:
        volumes = model.unpatchify(volumes)
        masks = masks.unsqueeze(-1).repeat(1, 1, vars_['reconstruct_imgs'].shape[-1])  # (N, H*W, p*p*3)
    ...
    for i in range(vol_n):
        ...
        # np.save(os.path.join(save_dir, vars_['img_names'][i], 'original.npy'), x.numpy())
        # # save mask
        # np.save(os.path.join(save_dir, vars_['img_names'][i], 'mask.npy'), mask.numpy())
        # # save vol_i
        # np.save(os.path.join(save_dir, vars_['img_names'][i], 'pred.npy'), vol_i.numpy())
        # # save im_masked
        # np.save(os.path.join(save_dir, vars_['img_names'][i], 'pred_paste.npy'), im_masked.numpy())
        ...
        for z in range(vol_i.shape[0]):
            ...
            if save_dir is not None:
                os.makedirs(os.path.join(save_dir, vars_['img_names'][i]), exist_ok=True)
                plt.savefig(os.path.join(save_dir, vars_['img_names'][i], f'frame_{z}.png'))
            else:
                buf = io.BytesIO()
                ...
                image_z.append(image)
            plt.close('all')
        if save_dir is None:
            images.append(image_z)
    return images if save_dir is None else None


def get_inpaint_images(vars_: dict, model, save_dir: str, n_cut: int = 2):
    ...
    samples = torch.index_select(vars_['samples'], 2,
            torch.linspace(
                0,
                vars_['samples'].shape[2] - 1,
                pred_t_dim,
            ).long().to(vars_['samples'].device))
    masks = torch.index_select(masks, 2,
            torch.linspace(
                0,
                masks.shape[2] - 1,
                pred_t_dim,
            ).long().to(masks.device))
