...
class DINOv2AsClassifier(nn.Module):
    def __init__(self, 
                 config_path: str,
                 num_classes: int=1, 
                 pretrained: str=None,
...
def dinov2_large_classifier(**kwargs):
    model = DINOv2AsClassifier(config_path='models/dinov2_modules/configs/train/vitl16_lbsz48_short.yaml', 
                               hidden_dim=1024, 
                               **kwargs)
    return model
...
if __name__ == '__main__':
    ...
    model = dinov2_large_classifier(pretrained=pretrained, num_classes=12).cuda()
