...
class DINOv2Mean(nn.Module):
    ...
    def forward(self, x):
        ...
        return logits, masks.sum(dim=1)  # return logits and the sum of masks for each video
...
def main():
        ...
        with tqdm(total=len(dataloader), desc="Processing videos", unit="batch") as pbar:
            for i, (video_with_mask, study, video_path) in enumerate(dataloader):
                ...
                for j in range(embedding.shape[0]):
                    ...
                    torch.save({"embedding": embedding[j].cpu(), "num_frames": num_frames[j].cpu()}, embedding_path)
                pbar.update(1)
    ...
    if args.mean_embeddings:
        ...
        for study in tqdm(studies, desc="Computing mean embeddings", unit="study"):
            ...
            sum_embeddings = torch.sum(torch.stack([embedding_dict["embedding"] for embedding_dict in embeddings_this_study_dict.values()], dim=0), dim=0)
            sum_frames = torch.stack([embedding_dict["num_frames"] for embedding_dict in embeddings_this_study_dict.values()], dim=0).sum()
            all_embeddings[study] = sum_embeddings / sum_frames
